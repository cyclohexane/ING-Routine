...
- 完善了协会官网后台管理系统的前四个功能。
- 今天发现后端同学用jq的ajax动态生成option，用了$(document).ready()，这样layui无法解析，所以最后生成的是系统自带的option。直接去掉$(document).ready()，就可以了。
...
- chooser.getFile()。参数是可选的但是非要我传一个参数，只能传'*'了
- chooser选文件后的file.data是uint8array格式的，不能直接添加进FormData。所以考虑配合filetransfer。通过chooser得到名称和路径，然后通过filetransfer上传。